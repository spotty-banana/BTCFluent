...
def electrum_check_incoming_txs():
    ...
    recent_transactions = \
    electrum_command("onchain_history", {"wallet": wallet_path, "from_height": old_blockcheight - 5})["transactions"]
            ...
            if AssetAddress.objects.filter(address=txaddress, account__isnull=False).exists() == True:
                ...
                if tx["confirmations"] >= 2 and inc_tx.transaction == None and inc_tx.confirmed_at == None:
                    ...
                    rows_updated_2 = IncomingTransaction.objects.filter(id=inc_tx.id, tx_identifier=TXidentifier,
                                                                        confirmed_at=None).update(
                        confirmed_at=datetime.datetime.now(), confirmations=tx["confirmations"])
                    if rows_updated_2 < 1:
                        raise Exception("Problem initiating tx, IncomingTransaction %d" % (inc_tx.id))

                    # add incoming address
                    IncomingTransaction.objects.filter(id=inc_tx.id, tx_identifier=TXidentifier, address=None).update(
                        address=incoming_address)
...
def electrum_process_outgoing_transactions():
    from accounts.models import Outgoingtransaction, Asset
    ...
    for tx in txorders:
        ...
        transaction_base64 = electrum_command("payto", {"wallet": wallet_path, "amount": BTC_amount,
                                                        "destination": tx.to_address, "unsigned": "true"})

        # update Outgoingtransaction base64
        Outgoingtransaction.objects.filter(id=tx.id, transaction_base64=None).update(
...
def electrum_process_fees():
    ...
    recent_transactions = \
    electrum_command("onchain_history", {"wallet": wallet_path, "from_height": scanned_to_height})["transactions"]
    ...
    for tx in recent_transactions:
        ...
        for i in txinfo:
            ...
            if Outgoingtransaction.objects.filter(to_address=txaddress).exists() == True:
                ...
                if tx["incoming"] == "false" and outgoing_transaction.fees_processed == False and tx[
                    "confirmations"] > 0:

                    # Take actual fees out of feeaccount balance
                    fee_old_balance = Asset.objects.get(ticker="BTC").fee_account.balance
                    rows_updated = Account.objects.filter(asset=Asset.objects.get(ticker="BTC"), is_fee_account=True,
                                                          balance=fee_old_balance).update(
                        balance=old_balance - tx["fee_sat"])
