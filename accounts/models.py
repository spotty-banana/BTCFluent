    ...
    def __str__(self):
        ...
        if self.from_account_id and self.to_account_id:
            if self.to_internal_address_id:
                desc = "Internal from %d to %d, through %s" % (
                self.from_account_id, self.to_account_id, self.to_internal_address.address)
            else:
...
class Account(models.Model):
    ...
    def get_incoming_in_timeframe(self, from_date, to_date):
        return (Transaction.objects.filter(to_account=self, created_at__range=[from_date, to_date]).aggregate(
            Sum('amount'))['amount__sum'] or 0)

    def get_outcoming_in_timeframe(self, from_date, to_date):
        return (Transaction.objects.filter(from_account=self, created_at__range=[from_date, to_date]).aggregate(
            Sum('amount'))['amount__sum'] or 0)

    def get_balance_before_date(self, from_date):
        latest_tx_before_date = Transaction.objects.filter(
            Q(from_account=self, created_at__lt=from_date) | Q(to_account=self, created_at__lt=from_date)).last()
    ...
    def get_txlist_data_in_timeframe(self, from_date, to_date):
        ...
        txs = Transaction.objects.filter(
            Q(to_account=self, created_at__range=[from_date, to_date]) | Q(from_account=self,
    ...
    def send_to_address(self, amount, to_address):
        ...
        if rows_updated == 1:
            ...
            rows_updated_2 = Account.objects.filter(id=fee_account.id,
                                                    balance=fee_account.balance).update(
                balance=F('balance') + self.asset.outgoing_tx_fee_amount)
    ...
    def get_new_address(self):
        address = AssetAddress.objects.filter(asset=self.asset, account=None, incomingtransaction=None).order_by(
            'created_at').first()
    ...
    def get_unused_address(self):
        address = AssetAddress.objects.filter(asset_id=self.asset, account_id=self.id, incomingtransaction=None,
                                              first_used_at=None).order_by('created_at').first()
        if not address:
...
class Outgoingtransaction(models.Model):
    ...
    def __str__(self):
        return ("To address:" + self.to_address + ", Amount:" + str(self.amount) + " Created at : " + str(
            self.created_at)) + " id: " + str(self.id)
